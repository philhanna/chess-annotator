...
* **Driving Adapters (Input)**:
    * **CLI Adapter**: Handles process lifecycle and launches the local application shell.
...
#### 2.1.1 PGN Ingestion
* When the user opens a file in the UI, the backend parses the entire PGN file into an in-memory collection of `chess.pgn.Game` objects.
...
* **File / Game Selection**: Provides controls to open a PGN file, show all games contained in that file, and switch the active game.
...
2.  **Startup**: CLI starts the local server and opens the SPA.
3.  **Open File**: User chooses a `.pgn` file from the UI.
...
6.  **Commitment**: User clicks **"Save"**, sending the updated in-memory PGN collection to the PGN Persistence Adapter.
7.  **Continuation**: User may switch to another game in the same file, open a different PGN file, or close the app and resume later by reopening a saved file.
